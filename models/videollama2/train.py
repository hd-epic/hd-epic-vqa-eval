...
class ModelArguments:
    ...
    model_type: Optional[str] = field(
        default="videollama2",
        metadata={
            "help": "Model type selected in the list: " + ", ".join(VLLMs.keys())
        },
    )
    model_path: Optional[str] = field(default="lmsys/vicuna-7b-v1.5")
    version: Optional[str] = field(
        default="v1", metadata={"help": "Version of the conversation template."}
    )
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        input_id = tokenizer_multimodal_token(
            conversation, tokenizer, modal_token, return_tensors="pt"
        )
    ...
    for i, source in enumerate(sources):
        ...
        for idx, sentence in enumerate(source):
            if idx % 2 == 1:
                tmp_message = [
                    {
                        "role": roles[source[idx - 1]["from"]],
                        "content": source[idx - 1]["value"],
                    },
                    {"role": roles[sentence["from"]], "content": sentence["value"]},
                ]

                instruction = tokenizer.apply_chat_template(
                    message + tmp_message[:1],
                    tokenize=False,
                ...
                instruction_len = len(
                    tokenizer_multimodal_token(
                        instruction, tokenizer, modal_token, return_tensors="pt"
                    )
                )
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        num_frames = (
            NUM_FRAMES
            if self.data_args.num_frames is None
            else self.data_args.num_frames
        )
        ...
        if "image" in sources[0]:
            ...
            try:
                ...
                print(
                    f"Encounted error when reading image {image_file}, use {backup_idx}-th example instead!!!"
                )
            ...
            try:
                ...
                backup_idx = random.randint(0, len(self.list_data_dict) - 1)
                print(
                    f"Encounted error when reading video {video_file}, use {backup_idx}-th example instead!!!"
                )
                return self.__getitem__(backup_idx)
        ...
        if self.data_args.is_pretraining:
            data_dict = preprocess_plain(
                sources, self.tokenizer, modal_token=modal_token
            )
        else:
        ...
        if "image" in self.list_data_dict[i]:
            ...
            data_dict["image"] = torch.zeros(
                3, self.data_args.image_size, self.data_args.image_size
            )
        return data_dict
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            for modal_token in MODAL_INDEX_MAP.keys():
                ...
                modal_name = re.findall("[<](.*)[>]", modal_token)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        data_args.video_processor = (
            vision_tower.video_processor
            if hasattr(vision_tower, "video_processor")
            else vision_tower.image_processor
        )
        ...
        model.config.num_frames = (
            NUM_FRAMES if data_args.num_frames is None else data_args.num_frames
        )
