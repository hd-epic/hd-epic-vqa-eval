...
def longest_repeating_substring(s):
    ...
    for i in range(1, n + 1):
        for j in range(i + 1, n + 1):
            if (dp[i - 1][j - 1] > 0 and dp[i - 1][j - 1] < (j - i)) or s[i - 1] == s[
                j - 1
            ]:
                dp[i][j] = dp[i - 1][j - 1] + 1
...
def main(args):
    ...
    for sample in pred_contents:
        ...
        new_sample = sample
        new_sample["video_name"] = (
            f"{video_id.split('/')[-1].split('.')[0]}_{video_id_counts[video_id]}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pred-path", required=True, help="The path to file containing prediction."
    )
    parser.add_argument(
        "--output-dir", required=True, help="The path to save annotation json files."
    )
    parser.add_argument(
        "--output-json",
        required=True,
    ...
    parser.add_argument(
        "--num-tasks", required=True, type=int, help="Number of splits."
    )
    parser.add_argument("--num_chunks", default=1, type=int, help="Result splits")
    parser.add_argument(
        "--api-key", required=True, type=str, help="Azure Openai API key."
    )
    parser.add_argument(
        "--api-endpoint", required=True, type=str, help="Azure Openai API endpoint."
    )
    parser.add_argument(
        "--api-deployname", required=True, type=str, help="Azure Openai API deployname."
    )
