    ...
    def __getitem__(self, idx):
        ...
        if video_path is None:
            raise FileNotFoundError(
                f"Video file not found for {os.path.join(args.video_folder, video_name)}"
            )
...
def run_inference(args):
    ...
    for i, (video_tensors, video_names, questions, question_ids, answers) in enumerate(
        tqdm(dataloader)
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video-folder", help="Directory containing video files.", required=True
    )
    parser.add_argument(
        "--question-file",
    ...
    parser.add_argument(
        "--output-file", help="Directory to save the model results JSON.", required=True
    )
