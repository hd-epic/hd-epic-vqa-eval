...
from videollama2.constants import LOGDIR
from videollama2.conversation import (
    default_conversation,
    conv_templates,
    SeparatorStyle,
)
from videollama2.utils import (
    build_logger,
    server_error_msg,
    violates_moderation,
    moderation_msg,
...
def add_text_ori(state, text, image, video, image_process_mode, request: gr.Request):
    ...
    assert image is None or video is None, (
        "Please don't feed image and video inputs at the same time!!!"
    )
...
def http_bot(
    ...
    if state.modality == "image" or state.modality == "image_text":
        ...
        all_images = state.get_videos(
            return_pil=True
        )  # return video frames where each frame is a PIL.Image object
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(
        title="Video-Llama", theme=gr.themes.Default(), css=block_css
    ) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                gr.Examples(
                    examples=[
                        ...
                        [
                            f"{cur_dir}/examples/1034346401.mp4",
            ...
            with gr.Column(scale=8):
                chatbot = gr.Chatbot(
                    elem_id="chatbot", label="Videollama2 Chatbot", height=550
                )
                with gr.Row():
                    with gr.Column(scale=8):
                        textbox.render()
