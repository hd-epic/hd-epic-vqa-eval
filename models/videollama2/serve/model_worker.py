...
class ModelWorker:
    def __init__(
        ...
        self.is_multimodal = (
            "videollama2" in self.model_name.lower() or "vlb" in self.model_name.lower()
        )
    ...
    def generate_stream(self, params):
        ...
        modal_list = []
        if (
            images_or_videos is not None
            and len(images_or_videos)
            and self.is_multimodal
        ):
            if len(images_or_videos) > 0:
                if len(images_or_videos) != prompt.count(DEFAULT_IMAGE_TOKEN) and len(
                    images_or_videos
                ) != (prompt.count(DEFAULT_VIDEO_TOKEN)):
                    raise ValueError(
                        "Number of images/videos does not match number of <image>/<video> tokens in prompt"
                    )

                try:
                    print("Load image...")
                    images_or_videos = [
                        load_image_from_base64(image) for image in images_or_videos
                    ]
                    images_or_videos = process_images(
                        images_or_videos, image_processor, model.config
                    )
                    ...
                    if "use_taug" not in self.model_path:
                        frame_id_list = np.linspace(0, duration - 1, 8, dtype=int)
                        video_frames = decord_vr.get_batch(frame_id_list).asnumpy()
                        images_or_videos = process_videos(
                            video_frames, image_processor, model.config
                        )
                    else:
                        print("Temporal augmentation activated!!!")
                        frame_id_list = np.linspace(
                            0, duration - 1, 8 * 2 * 2, dtype=int
                        )
                        video_data = decord_vr.get_batch(frame_id_list)
                        video_frames = [
                            Image.fromarray(f) for f in video_data.asnumpy()
                        ]
                        chunked_video_frames = chunk_list(video_frames, 2 * 2)
                        expanded_video_frames = [
                            frame_expansion(frame_list, 2)
                            for frame_list in chunked_video_frames
                        ]
                        images_or_videos = process_videos(
                            expanded_video_frames, image_processor, model.config
                        )
                ...
                if type(images_or_videos) is list:
                    images_or_videos = [
                        ...
                        for image in images_or_videos
                    ]
                else:
                    images_or_videos = images_or_videos.to(
        ...
        for new_text in streamer:
            ...
            if token_count >= STREAM_CHECK_MULTIPLE:
                ...
                if safety_message:
                    ...
                    yield (
                        json.dumps({"text": safety_message, "error_code": 1}).encode()
                        + b"\0"
                    )
                    return
                token_count = 0  #
