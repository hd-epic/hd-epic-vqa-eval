...
class Chat:
    def __init__(self, model_path, load_8bit=False, load_4bit=False):
        ...
        self.model, self.processor, self.tokenizer = model_init(
            model_path, load_8bit=load_8bit, load_4bit=load_4bit
        )
...
def generate(
    ...
    try:
        if image is not None:
            data.append(
                (
                    processor["image"](image).to(handler.model.device, dtype=dtype),
                    "<image>",
                )
            )
            ...
            data.append(
                (
                    processor["video"](video).to(handler.model.device, dtype=dtype),
                    "<video>",
                )
            )
    ...
    assert len(message) % 2 == 0, (
        "The message should be a pair of user and system message."
    )
    ...
    # 2. not first run case
        ...
        for i in range(length - 1, -1, -1):
            ...
            previous_video = re.findall(
                r'<video controls playsinline width="500" style="display: inline-block;"  src="./file=(.+?)"',
                chatbot[i][0],
            )
            ...
            if len(previous_image) > 0:
                ...
                if (video is not None) or (
                    image is not None
                    and os.path.basename(previous_image) != os.path.basename(image)
                ):
                    message.clear()
                    one_turn_chat[0] += "\n" + show_images
                break
                ...
                if image is not None or (
                    video is not None
                    and os.path.basename(previous_video) != os.path.basename(video)
                ):
                    message.clear()
                    one_turn_chat[0] += "\n" + show_images
                break
...
with gr.Blocks(title="VideoLLaMA 2 🔥🚀🔥", theme=theme, css=block_css) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=7):
            chatbot = gr.Chatbot(
                label="VideoLLaMA 2", bubble_full_width=True, height=750
            )
            with gr.Row():
    ...
    regenerate_btn.click(regenerate, [message, chatbot], [message, chatbot]).then(
        generate,
        [
            image,
            video,
    ...
    clear_btn.click(
        clear_history, [message, chatbot], [image, video, message, chatbot, textbox]
    )
