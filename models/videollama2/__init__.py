...
from .mm_utils import depic_process_video
...
def mm_infer(image_or_video, instruct, model, tokenizer, modal="video", **kwargs):
    ...
    attention_masks = (
        input_ids.ne(tokenizer.pad_token_id).long().cuda()
    )  # 1 for real tokens, 0 for padding tokens.

    # 3. generate response according to visual signals and prompts.
    keywords = [
        tokenizer.eos_token
    ]  # stop generation when the model predicts the end of the sentence '</s>'
...
def depic_model_infer(input, model, tokenizer, config, **kwargs):
    modal_token = DEFAULT_VIDEO_TOKEN

    assert len(input) == 3, f"Expected 3 inputs, got {len(input)}"

    video_num, video_data, instruct = input
    video_data = video_data.half().cuda()
    ...
    system_message = [{"role": "system", "content": config["prompts"]["sys"]}]
