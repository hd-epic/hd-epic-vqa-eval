...
def depic_process_video(
    video_data, processor, s=None, e=None, aspect_ratio="pad", num_frames=NUM_FRAMES
):
    video_data = [Image.fromarray(f) for f in video_data]

    # Pad upto num_frames with black frames
    while num_frames is not None and len(video_data) < num_frames:
        video_data.append(
            Image.fromarray(np.zeros((*video_data[-1].size, 3), dtype=np.uint8))
        )
    ...
    if len(video_data) > MAX_FRAMES:
        print(
            f"Warning: Truncating video input from {len(video_data)} to {MAX_FRAMES} frames."
        )
        video_data = video_data[:MAX_FRAMES]
...
def tokenizer_multimodal_token(
    ...
    """
    multimodal_token_index = MODAL_INDEX_MAP.get(
        multimodal_token, None
    )  # {'<image>': -200, '<video>': -201, '<audio>': -202}
    if multimodal_token_index is None:
        ...
        # here we add multimodal token between each chunk
