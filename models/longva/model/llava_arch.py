...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        image_features = self.get_model().vision_resampler(
            image_features, images=images
        )
            ...
            if mm_patch_merge_type == "flat":
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        kernel_size = mm_patch_merge_type.split("avgpool")[-1].split(
                            "x"
                        )[-1]
                        kernel_size = 2
                        image_feature = image_feature.view(
                        ...
                        image_feature = image_feature.permute(
                            0, 3, 1, 2
                        ).contiguous()  # [4, 4096, 24, 24]
                        image_feature = nn.functional.avg_pool2d(
                            image_feature, kernel_size
                        )  # [4, 4096, 12, 12]
                        image_feature = image_feature.flatten(2, 3)  # [4, 4096, 144]
                        image_feature = image_feature.permute(
                            0, 2, 1
                        ).contiguous()  # [4, 144, 4096]
